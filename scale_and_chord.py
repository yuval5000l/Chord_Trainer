...
def scale_chord_maker():
    root = random.randrange(C_LOWEST, C_LOWEST + 13)
    scale_name = random.choice(list(SCALES_DICTIONARY.keys()))
    chord_num = random.randrange(1, 8)
    chord_root, chord_name = SCALES_CHORDS_DICTIONARY[scale_name][
        chord_num - 1]
    chord_root += root
    notes_in_chord = [(chord_root + i) % 12 for i in
                      CHORDS_DICTIONARY[chord_name]]
    # print(SCALES_CHORDS_DICTIONARY[scale_name][
    #     chord_num - 1])c

    # print(chord_name, note_check(chord_root))
    return notes_in_chord, chord_num, str(
        note_check(root)) + ' ' + scale_name
        ...
        for e in events:
            ...
            if e.type == pg.KEYDOWN:
                ...
                if e.key == pg.K_c:
                    ...
                    if chord_game:
                        if not cur_chord:
                            cur_chord = scale_chord_maker()
                        if cur_chord:
                            print(f"Our current scale is "
                                  f"{(cur_chord[2])} "
                                  f"\nYour chord is {cur_chord[1]}")
                if e.key == pg.K_h:
                    if chord_game:
                        print((note_check(cur_chord[0][0]+C_LOWEST),
                               note_check(cur_chord[0][1]+C_LOWEST),
                               note_check(cur_chord[0][2]+C_LOWEST)))
